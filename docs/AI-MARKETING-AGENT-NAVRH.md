...
## 13. Správa více účtů (Multi-Account)

### 13.1 Přehled

Systém je navržen pro budoucí rozšíření na správu více Facebook účtů/stránek z jednoho rozhraní.

```
┌─────────────────────────────────────────────────────────────────┐
│                     MULTI-ACCOUNT ARCHITEKTURA                   │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │                    MASTER DASHBOARD                          ││
│  │  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐        ││
│  │  │ Účet 1  │  │ Účet 2  │  │ Účet 3  │  │  + Nový │        ││
│  │  │Svobodné │  │ Reality │  │  Klient │  │   účet  │        ││
│  │  │ Reality │  │  Praha  │  │    X    │  │         │        ││
│  │  └────┬────┘  └────┬────┘  └────┬────┘  └─────────┘        ││
│  └───────┼────────────┼────────────┼───────────────────────────┘│
│          │            │            │                             │
...
### 13.2 Datový model pro více účtů

```typescript
// Organizace (zastřešuje všechny účty)
...
interface Account {
  ...
  facebookPageId: string
  facebookAccountId: string
  facebookAccessToken: string  // Šifrovaný
  facebookPageAccessToken: string  // Šifrovaný
...
// Rozšířená konfigurace agenta
interface AgentConfig {
  accountId: string

  // ... existující položky ...

  // Multi-account specifické
  timezone: string  // Pro každý účet může být jiná
  language: 'cs' | 'en' | 'sk'
  currency: 'CZK' | 'EUR' | 'USD'
}
```

### 13.3 Přepínání mezi účty

```
┌─────────────────────────────────────────────────────────────────┐
│  🤖 AI Marketing Agent              [Svobodné Reality ▼] [⚙️]   │
├─────────────────────────────────────────────────────────────────┤
│                                  ┌─────────────────────────────┐│
│                                  │ 📋 Vyberte účet:            ││
│                                  ├─────────────────────────────┤│
│                                  │ ✓ Svobodné Reality          ││
│                                  │     15,420 reach | 12 leads ││
│                                  │ ─────────────────────────── ││
│                                  │   Reality Praha             ││
│                                  │     8,200 reach | 5 leads   ││
│                                  │ ─────────────────────────── ││
...
│                                                                  │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │ Celkový rozpočet:    25,000 Kč    Utraceno: 5,420 Kč      │  │
│  │ Celkový reach:       38,640       Celkem leads: 24        │  │
│  │ Aktivní kampaně:     8            Čekající schválení: 5   │  │
│  └───────────────────────────────────────────────────────────┘  │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │ Účet              Status    Budget      Reach      Leads   ││
│  ├─────────────────────────────────────────────────────────────┤│
│  │ Svobodné Reality   ✅       8,150 Kč    15,420     12      ││
│  │ Reality Praha      ✅       4,800 Kč     8,200      5      ││
│  │ Klient X           ⏸️          -            -       -      ││
│  │ ─────────────────────────────────────────────────────────── ││
│  │                    Celkem   12,950 Kč   23,620     17      ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                  │
│  [Zobrazit detail všech] [Exportovat report] [Hromadná akce]    │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
| Aspekt | Izolace | Popis |
|--------|---------|-------|
| **Database** | Sdílená DB, oddělené tabulky | `account_id` v každém záznamu |
| **Tokeny** | Šifrované, per-account | AES-256 encryption |
| **Rozpočet** | Kompletně oddělený | Nelze "přelévat" mezi účty |
| **Historie** | Oddělená | Audit log per account |
| **AI kontext** | Oddělený | Každý účet má vlastní strategii |

### 13.6 API rozšíření pro multi-account

```
# Organizace
GET  /api/organization                     # Info o organizaci
GET  /api/organization/accounts            # Seznam všech účtů

# Účty
POST /api/accounts                         # Přidat nový účet
GET  /api/accounts/:id                     # Detail účtu
PUT  /api/accounts/:id                     # Upravit účet
DELETE /api/accounts/:id                   # Odebrat účet
POST /api/accounts/:id/connect             # Připojit Facebook
POST /api/accounts/:id/disconnect          # Odpojit Facebook

# Přepínání kontextu
POST /api/context/switch/:accountId        # Přepnout aktivní účet

# Agregované reporty
GET  /api/reports/all-accounts             # Souhrnný report
GET  /api/reports/compare                  # Porovnání účtů
```

### 13.7 Přidání nového účtu - workflow
...
│  ┌─────────────────────────────────────────────────────────────┐│
│  │ Název účtu:        [Reality Praha                        ]  ││
│  │ Popis:             [Realitní kancelář v Praze            ]  ││
│  │ Časová zóna:       [Europe/Prague               ▼]          ││
│  │ Měna:              [CZK ▼]                                  ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                     [Pokračovat]│
│                                                                  │
│  Krok 2: Připojení Facebooku                                    │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │                                                              ││
│  │      [🔵 Připojit přes Facebook]                            ││
│  │                                                              ││
│  │      Budete přesměrováni na Facebook pro autorizaci.        ││
│  │      Agent potřebuje oprávnění:                             ││
│  │      ✓ Správa stránek                                       ││
│  │      ✓ Správa reklam                                        ││
│  │      ✓ Přístup k insights                                   ││
│  │                                                              ││
...
│  ┌─────────────────────────────────────────────────────────────┐│
│  │ Měsíční rozpočet:   [10000] Kč                              ││
│  │ Denní limit:        [500] Kč                                ││
│  │ Cílová skupina:     [Lidé hledající byty v Praze         ]  ││
│  │ Frekvence postů:    [Každý pracovní den ▼]                  ││
│  │ Email pro reporty:  [jan@example.cz                      ]  ││
│  └─────────────────────────────────────────────────────────────┘│
│                                     [Zpět] [Vytvořit účet]      │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
...
### 13.8 Cenový model (budoucí)

| Plán | Cena | Max účtů | Funkce |
|------|------|----------|--------|
| **Free** | 0 Kč/měs | 1 | Základní funkce |
| **Basic** | 499 Kč/měs | 3 | + Pokročilé reporty |
| **Pro** | 1,499 Kč/měs | 10 | + API přístup, priority support |
| **Enterprise** | Dohodou | Neomezeno | + Custom integrace, SLA |

### 13.9 Migrace ze single-account

Pro přechod z jednoúčtové verze:

1. **Zachování kompatibility** - stávající API funguje beze změn
2. **Automatická migrace** - stávající účet se stane prvním účtem v organizaci
3. **Volitelné rozšíření** - multi-account se aktivuje až při přidání druhého účtu

```typescript
// Middleware pro backward compatibility
const accountContext = (req, res, next) => {
  // Pokud není specifikován účet, použij výchozí
  if (!req.headers['x-account-id']) {
    req.accountId = getDefaultAccountId(req.userId);
  } else {
