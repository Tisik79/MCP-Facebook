...
import { DEFAULT_FB_APP } from './default-app.js';
...
export interface AdAccountInfo { name: string; currency: string; }
export interface ActiveSelection { pageId?: string; adAccountId?: string; }
...
export function getAppCredentials(): { appId: string; appSecret: string; source: AppCredsSource } {
  ...
  return { ...DEFAULT_FB_APP, source: 'builtin' };
}
...
export function saveAppCredentials(appId: string, appSecret: string): void {
  fs.writeFileSync(CONFIG_FILE_PATH, JSON.stringify({ appId, appSecret }, null, 2));
}

function pageEntries(tokens: TokenStore): [string, PageToken][] {
  return Object.entries(tokens).filter(([k]) => !k.startsWith('_')) as [string, PageToken][];
}

function normAct(id?: string): string | undefined {
...
export function getToken(pageId?: string): string {
  ...
  if (pages.length > 0) return pages[0][1].access_token;
  throw new Error('Žádný Facebook token není k dispozici. V Claude řekni: „Připoj Facebook účet".');
}

export function getActivePageId(): string | undefined {
  const tokens = loadTokens();
  if (tokens._active?.pageId && tokens[tokens._active.pageId]) return tokens._active.pageId;
  const pages = pageEntries(tokens);
  return pages.length > 0 ? pages[0][0] : undefined;
}

export function getActivePage(): { id: string; name: string; access_token: string } | null {
  const tokens = loadTokens();
  const id = getActivePageId();
  if (!id || !tokens[id]) return null;
  const p = tokens[id] as PageToken;
...
export function getAdAccountId(): string {
  ...
  if (active && tokens._ad_accounts && tokens._ad_accounts[active]) return active;
  const ids = Object.keys(tokens._ad_accounts || {});
  if (ids.length > 0) return normAct(ids[0])!;
  return process.env.FACEBOOK_ACCOUNT_ID ? normAct(process.env.FACEBOOK_ACCOUNT_ID)! : '';
}

export function listConnectedPages(): { id: string; name: string; category?: string; active: boolean }[] {
  const tokens = loadTokens();
...
export function listConnectedAdAccounts(): { id: string; name: string; currency: string; active: boolean }[] {
  const tokens = loadTokens();
  const activeId = getAdAccountId();
  return Object.entries(tokens._ad_accounts || {}).map(([id, data]) => ({
    id, name: (data as AdAccountInfo).name, currency: (data as AdAccountInfo).currency, active: normAct(id) === activeId,
  }));
}

/** Nastaví aktivní (výchozí) stránku a/nebo reklamní účet. Vrací aktuální stav. */
export function setActiveSelection(sel: ActiveSelection): { pageId?: string; adAccountId?: string } {
  const tokens = loadTokens();
  const cur = tokens._active || {};
  const next: ActiveSelection = { ...cur };
  if (sel.pageId !== undefined) {
    if (sel.pageId && !tokens[sel.pageId]) throw new Error('Stránka ' + sel.pageId + ' není propojena.');
    next.pageId = sel.pageId || undefined;
  }
  if (sel.adAccountId !== undefined) {
    const a = normAct(sel.adAccountId);
    if (a && !(tokens._ad_accounts && tokens._ad_accounts[a])) throw new Error('Reklamní účet ' + a + ' není propojen.');
    next.adAccountId = a;
  }
  tokens._active = next;
  saveTokens(tokens);
...
async function fetchAndStore(longToken: string, expires?: number): Promise<{ pages: string[]; adAccounts: string[] }> {
  ...
  if (pagesData.error) throw new Error('me/accounts: ' + pagesData.error.message);
  if (adData.error) throw new Error('me/adaccounts: ' + adData.error.message);
  ...
  const firstPageId = (pagesData.data || [])[0]?.id as string | undefined;
  const firstAdId = (adData.data || [])[0]?.id as string | undefined;
...
export async function refreshUserTokenIfNeeded(appId: string, appSecret: string, refreshWithinMs = 14 * 24 * 60 * 60 * 1000): Promise<boolean> {
  const tokens = loadTokens();
  const cur = tokens._user;
  if (!cur?.access_token) return false;
  const msLeft = (cur.expires || 0) - Date.now();
  if (msLeft > refreshWithinMs) { reinitSdk(); return false; } // ještě dost času
  try {
...
export async function setUserToken(appId: string, appSecret: string, providedToken: string): Promise<{ pages: string[]; adAccounts: string[]; longLived: boolean }> {
  ...
  if (!token) throw new Error('Prázdný token.');
...
export function getOAuthDialogUrl(appId: string): string {
...
export function startAuthServer(appId: string, appSecret: string): void {
  ...
  server.on('error', (err: any) => {
    if (err.code === 'EADDRINUSE') process.stderr.write('[Facebook MCP] Port ' + AUTH_PORT + ' je obsazen – auth server už pravděpodobně běží.\n');
    else process.stderr.write('[Facebook MCP] Auth server chyba: ' + err.message + '\n');
  });
}
