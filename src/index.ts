...
const initializeServer = async (): Promise<McpServer> => {
  ...
  server.tool(
    'connect_facebook_account',
    { force: z.boolean().optional().describe('Prinutit nove prihlaseni i kdyz uz je ucet propojen') },
    async ({ force }) => {
      ...
      const result = await new Promise<string>((resolve, reject) => {
        const server = createServer(async (req, res) => {
          ...
          try {
            ...
            if (!existing._ad_accounts) existing._ad_accounts = {};
            ...
            res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
            res.end('<html><body style="font-family:sans-serif;max-width:500px;margin:60px auto;padding:20px"><h1 style="color:#1a7a4a">Prihlaseni uspesne!</h1><p>Propojeno stranek: ' + pageNames.length + '</p><ul>' + pageNames.map((n: string) => '<li>' + n + '</li>').join('') + '</ul><p><strong>Muzete zavrit toto okno a vratit se do Claude.</strong></p></body></html>');

            server.close();
            resolve('Uspesne propojeno ' + pageNames.length + ' stranek:\n' + pageNames.map((n: string) => '  - ' + n).join('\n'));
          } catch (err: any) {
        ...
        server.on('error', (err: any) => {
          if (err.code === 'EADDRINUSE') reject(new Error('Port ' + AUTH_PORT + ' je obsazen. Zkus znovu za chvili.'));
          else reject(err);
        });
