...
export function saveConfig(cfg: FbConfig): void {
...
export async function runSetupWizard(): Promise<FbConfig> {
  ...
  const cfg: FbConfig = { appId, appSecret };
  saveConfig(cfg);
  rl.close();
...
export function runOAuthFlow(appId: string, appSecret: string): Promise<void> {
