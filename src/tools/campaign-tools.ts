...
export const createCampaign = async (
  ...
  bidStrategy?: string // Strategie nabídek na úrovni kampaně (CBO)
) => {
  try {
...
export const getCampaigns = async (limit = 10, status?: string) => {
  try {
    ...
    const params: any = { limit: status ? Math.max(limit, 100) : limit };

    // Získání kampaní
    const campaigns = await adAccount.getCampaigns(fields, params);

    let mapped = campaigns.map((campaign: any) => ({
...
export const updateCampaign = async (
  ...
  try {
    ...
    if (dailyBudget) params.daily_budget = String(dailyBudget * 100);
    ...
    if (!res.ok || data?.error) {
      return { success: false, message: formatFbError(data, 'Chyba při aktualizaci kampaně') };
    }
    ...
    if (v?.error) {
      return { success: false, message: formatFbError(v, 'Chyba při ověření aktualizace kampaně') };
    }

    const mismatch: string[] = [];
    if (name && v.name !== name) mismatch.push(`název (požadováno "${name}", je "${v.name}")`);
    ...
    return {
      success: false,
      message: formatFbError(error, 'Chyba při aktualizaci kampaně')
    };
  }
};
...
export const getCampaignDetails = async (campaignId: string) => {
  try {
    ...
    const d: any = campaignDetails._data || {};
    ...
    try {
      const ins: any[] = await campaign.getInsights(['spend'], { date_preset: 'today' });
      if (ins && ins.length) spentToday = parseFloat(ins[0]._data?.spend ?? ins[0].spend ?? '0');
